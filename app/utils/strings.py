...
def sanitize_text_for_subtitles(text: str) -> str:
    """
    Remove caracteres de entonação/prosódia que são úteis para TTS mas poluem legendas.

    Remove:
    - Reticências (...) → espaço
    - Travessões (—, –, -) quando usados como pausas → espaço
    - Múltiplos espaços → espaço único
    - Aspas decorativas
    - Tags de entonação do ElevenLabs v3 (ex: <break>, [pause])
    ...
    Args:
        text: Texto original com marcadores de entonação

    Returns:
        Texto limpo para exibição em legendas
    """
    # Remove tags do ElevenLabs v3 (se houver)
    text = re.sub(r'<[^>]+>', '', text)
    text = re.sub(r'\[[^\]]+\]', '', text)

    # Reticências no final de frase (antes de maiúscula ou fim) → ponto
    text = re.sub(r'\.{2,}\s*([A-ZÁÉÍÓÚÂÊÎÔÛÃÕÇ])', r'. \1', text)
    text = re.sub(r'…\s*([A-ZÁÉÍÓÚÂÊÎÔÛÃÕÇ])', r'. \1', text)

    # Reticências restantes → espaço
    text = re.sub(r'\.{2,}', ' ', text)
    text = re.sub(r'…', ' ', text)

    # Remove travessões usados como pausas (— ou -- ou –)
    # Mantém hífen simples em palavras compostas (ex: "bem-vindo")
    text = re.sub(r'\s*[—–]\s*', ' ', text)  # Travessão com espaços
    text = re.sub(r'\s*--\s*', ' ', text)     # Duplo hífen com espaços

    # Remove aspas decorativas (mas mantém aspas simples em contrações)
    text = re.sub(r'[""„‟]', '', text)
...
def sanitize_sentences_for_subtitles(sentences: list[str]) -> list[str]:
    """
    Aplica sanitização a uma lista de sentenças.

    Args:
        sentences: Lista de sentenças originais (para TTS)

    Returns:
        Lista de sentenças limpas (para legendas)
    """
    return [sanitize_text_for_subtitles(s) for s in sentences]
